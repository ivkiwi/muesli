...
public final class DictationStore {
    ...
    private let diagnosticsLogger: ((String) -> Void)?
    ...
    public init(databaseURL: URL, diagnosticsLogger: ((String) -> Void)? = nil) {
    ...
    public func migrateIfNeeded() throws {
        ...
        if sqlite3_exec(db, "ALTER TABLE meetings ADD COLUMN raw_original_transcript TEXT", nil, nil, nil) != SQLITE_OK {
            // Column may already exist.
        }
        ...
        try repairNumericMeetingTimestamps(db: db)
        try migrateMeetingCalendarEventIndex(db: db)
    ...
    public func savedRecordingReferenceCount(path: String, excludingMeetingID: Int64? = nil) throws -> Int {
        ...
        WHERE saved_recording_path IN (?, ?) AND deleted_at IS NULL
        """
    ...
    public func meetingByCalendarEventID(_ calendarEventID: String, startTime: Date) throws -> MeetingRecord? {
        let db = try openDatabase()
        defer { sqlite3_close(db) }
    ...
    public func updateMeetingTranscript(id: Int64, rawTranscript: String) throws {
        ...
        let sql = "UPDATE meetings SET raw_transcript = ?, raw_original_transcript = NULL, word_count = ?, updated_at = ?, sync_dirty = 1 WHERE id = ?"
        var statement: OpaquePointer?
        guard sqlite3_prepare_v2(db, sql, -1, &statement, nil) == SQLITE_OK else {
            throw lastError(db)
        }
    ...
    public func recoverLiveMeetingFromTranscriptCheckpoints(id: Int64) throws -> Bool {
        ...
        let sql = """
        UPDATE meetings
        SET end_time = ?, duration_seconds = ?, raw_transcript = ?, raw_original_transcript = NULL, formatted_notes = ?, meeting_status = ?, word_count = ?, updated_at = ?, sync_dirty = 1
        WHERE id = ?
        """
        var statement: OpaquePointer?
        guard sqlite3_prepare_v2(db, sql, -1, &statement, nil) == SQLITE_OK else {
    ...
    private func makeMeetingRecord(_ statement: OpaquePointer?) -> MeetingRecord {
        let rawOriginalTranscript: String? = sqlite3_column_type(statement, 5) == SQLITE_NULL ? nil : stringColumn(statement, index: 5)
        let folderID: Int64? = sqlite3_column_type(statement, 8) == SQLITE_NULL ? nil : sqlite3_column_int64(statement, 8)
        let calendarEventID: String? = sqlite3_column_type(statement, 9) == SQLITE_NULL ? nil : stringColumn(statement, index: 9)
        let micAudioPath: String? = sqlite3_column_type(statement, 10) == SQLITE_NULL ? nil : stringColumn(statement, index: 10)
        let systemAudioPath: String? = sqlite3_column_type(statement, 11) == SQLITE_NULL ? nil : stringColumn(statement, index: 11)
        let savedRecordingPath: String? = sqlite3_column_type(statement, 12) == SQLITE_NULL ? nil : stringColumn(statement, index: 12)
        ...
        return MeetingRecord(
            ...
            formattedNotes: stringColumn(statement, index: 6),
            wordCount: Int(sqlite3_column_int(statement, 7)),
            folderID: folderID,
    ...
    private func repairNumericMeetingTimestamps(db: OpaquePointer?) throws {
        try repairNumericMeetingTimestampColumn("start_time", db: db)
        try repairNumericMeetingTimestampColumn("end_time", db: db)
    }

    private func repairNumericMeetingTimestampColumn(_ column: String, db: OpaquePointer?) throws {
        let sql = """
        UPDATE meetings
        SET \(column) = strftime('%Y-%m-%dT%H:%M:%SZ', CAST(\(column) AS REAL), 'unixepoch'),
            updated_at = strftime('%s', 'now'),
            sync_dirty = 1
        WHERE \(column) IS NOT NULL
          AND trim(CAST(\(column) AS TEXT)) NOT LIKE '%-%'
          AND CAST(\(column) AS REAL) BETWEEN 978307200 AND 4102444800
          AND strftime('%Y-%m-%dT%H:%M:%SZ', CAST(\(column) AS REAL), 'unixepoch') IS NOT NULL
        """
        try exec(sql, db: db)
    }
